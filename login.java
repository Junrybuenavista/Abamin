...
public class login 
        {
        	...
        	Statement st;
        	ResultSet set;
        	public String getUser(){return txt1.getText();}
              		...
              		if(result==0)
               		{
           	 	  		try
           	 	  		{
           	 	  		    set=st.executeQuery("select count(*) as me from tblLogin where Username='"+txt1.getText()+"'");
           	 	  		    set.next();
           	 	  		    if(Integer.parseInt(set.getString("me"))==0)JOptionPane.showMessageDialog(fr,"User Not Found !");
           	 	  		    else 
           	 	  		    {
           	 	  		    	set=st.executeQuery("select password from tblLogin where Username='"+txt1.getText()+"'");
           	 	  		        set.next();
           	 	  		        if(!set.getString("password").equals(pass.getText()))JOptionPane.showMessageDialog(fr,"Incorrect Password Please Try Again !");
           	 	  		        else break;
           	 	  		    }
           	 	  		}
           	 	  		catch(Exception ee){}
